...
function Occupancy({ capacity, current }: { capacity?: string; current?: string }) {
  ...
  const tone = ratio >= 0.9 ? "bg-rose-500" : ratio >= 0.7 ? "bg-amber-500" : "bg-emerald-500";
  return (
    <div className="mt-2">
            ...
            {data.shelters.map((sh) => (
              <Card key={sh.id} className="px-4 py-4 sm:px-5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="text-[15px] font-semibold text-neutral-900">{sh.name}</h3>
                ...
                <div className="mt-3 flex gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 px-3 py-1.5 text-[12.5px] font-medium text-neutral-700 transition-colors hover:border-neutral-300 hover:bg-neutral-50"
                  >
                    📍 地図を開く
                  </a>
                </div>
              ...
              {contacts.map((c) => (
                <li key={c.id} className="py-3">
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <span className="text-[14px] font-semibold text-neutral-900">{c.org}</span>
                    {c.category && (
                      <span className="rounded-md bg-neutral-100 px-1.5 py-0.5 text-[11.5px] font-medium text-neutral-500">
                        {c.category}
                      </span>
                    )}
                    {c.shelterId && (
                  ...
                  <div className="mt-0.5 flex flex-wrap gap-x-4 gap-y-0.5 text-[13px] text-neutral-500">
                  ...
                  {c.note && <p className="mt-1 text-[12.5px] text-neutral-400">{c.note}</p>}
                </li>
              ))}
            </ul>
          )}
