...
**列定義の正は `lib/relief/schema.ts`**（読み取り・書き込み・認証層・seed CSV 検証がすべて参照する
単一情報源）。列を変えるときは schema.ts と `docs/seed/*.csv` を直し、`npm run check:schema` で
一致を確認する。下表は人間向けの説明。
...
1. **ホーム** — 状況チップ（いまの状況）＋タイムライン（上=予定／「いま」区切り／下=実績・固定高スクロール）＋ネクストアクション（未対応要請・到着確認・直近予定・未共有下書きを自動導出）
...
4K・大型ディスプレイで全員が同時に見る「操作しない」統合ボード。45秒ごとに自動更新
（LIVE表示・最終更新時刻つき）。要請ボード（未対応/手配中）・物資パイプライン
（手配中/輸送中/本日到着）・避難所の収容状況・本日の動き・ライブタイムライン・
最新記録のティッカーを1画面に集約する。データはスマホUIと同じ /api/relief/data。
認証も同一（表示用に members へ専用アカウントを1行足す運用を推奨）。

### タブ間の相互リンク

各レコードの関連情報（避難所名・担当者名・要請⇄品目・タイムライン行など）は `RefLink`／行タップで
相互にジャンプできる。仕組み: `lib/relief/nav.ts` の `NavTarget`（tab / seg / focusId / memberId）を
ReliefApp の `navigate()` に渡す → タブ切替＋各ビューがセグメント・絞り込みを合わせ、
DOM id `rec-<レコードID>` へスクロールして一時ハイライト（`useFocusFlash`）。
ID 接頭辞（M-/G-/SC-/B-/SP-/R-/SH-/C-/L-）で所属タブを判別できる。
